...
import { useClientes, useCriarCliente, useAtualizarCliente, useRemoverCliente } from '../clientes/useClientes';
import type { Cliente } from '../clientes/useClientes';
    ...
    <div>
      ...
      {mostrarForm && (
        <form onSubmit={handleSalvar} className="bg-white rounded-lg shadow p-6 mb-6">
          {editandoId && <p className="text-sm text-blue-600 mb-3">Editando cliente...</p>}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {CAMPOS.map(({ label, campo, required }) => (
              <div key={campo}>
          ...
          <div className="flex gap-2 mt-4">
            <button type="submit" disabled={criar.isPending || atualizar.isPending}
              ...
              {(criar.isPending || atualizar.isPending) ? 'Salvando...' : editandoId ? 'Atualizar Cliente' : 'Salvar Cliente'}
            </button>
            {editandoId && (
            ...
            {clientes?.map((c: Cliente) => (
              <tr key={c.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right space-x-3">
                  ...
                  <button onClick={() => { if (confirm('Remover este cliente?')) remover.mutate(c.id); }}
